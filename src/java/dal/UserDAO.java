...
public class UserDAO {
    ...
    public void UpdateProfile(String username, String name, int phone, boolean gender, String email) throws SQLException {
        String sql = "UPDATE `doctris_system`.`users` SET `name` = ?, `phone` = ?, `gender` = ?, `email` = ? WHERE (`username` = ?)";
        try {
            connection = dbc.getConnection();
            ps = connection.prepareStatement(sql);
            ps.setString(1, name);
